...
void free_ntuple_list(ntuple_list in)
{
    if ( in == NULL || in->values == NULL )
        printf("free_ntuple_list: invalid n-tuple input.");
    free( (void *) in->values );
    free( (void *) in );
}
...
ntuple_list new_ntuple_list(unsigned int dim)
{
    ...
    /* check parameters */
    if ( dim == 0 ) printf("new_ntuple_list: 'dim' must be positive.");

    /* get memory for list structure */
    n_tuple = (ntuple_list) malloc( sizeof(struct ntuple_list_s) );
    if ( n_tuple == NULL ) printf("not enough memory.");
    ...
    if ( n_tuple->values == NULL ) printf("not enough memory.");
...
void enlarge_ntuple_list(ntuple_list n_tuple)
{
    ...
    if ( n_tuple == NULL || n_tuple->values == NULL || n_tuple->max_size == 0 )
        printf("enlarge_ntuple_list: invalid n-tuple.");
...
{
    /* check parameters */
    if ( out == NULL ) printf("add_5tuple: invalid n-tuple input.");
    if ( out->dim != 5 ) printf("add_5tuple: the n-tuple must be a 5-tuple.");

    /* if needed, alloc more tuples to 'out' */
    if ( out->size == out->max_size ) enlarge_ntuple_list(out);
    if ( out->values == NULL ) printf("add_5tuple: invalid n-tuple input.");
...
void free_image_char(image_char i)
{
    if ( i == NULL || i->data == NULL )
        printf("free_image_char: invalid input image.");
    free( (void *) i->data );
    free( (void *) i );
}
...
image_char new_image_char(unsigned int xsize, unsigned int ysize)
{
    ...
    /* check parameters */
    if ( xsize == 0 || ysize == 0 ) printf("new_image_char: invalid image size.");
    ...
    if ( image->data == NULL ) printf("not enough memory.");
                               ...
                               unsigned char fill_value )
{
    ...
    if ( image == NULL || image->data == NULL )
        printf("new_image_char_ini: invalid image.");
...
{
    if ( i == NULL || i->data == NULL )
        printf("free_image_int: invalid input image.");
    free( (void *) i->data );
    free( (void *) i );
}
...
image_int new_image_int(unsigned int xsize, unsigned int ysize)
{
    ...
    /* check parameters */
    if ( xsize == 0 || ysize == 0 ) printf("new_image_int: invalid image size.");
...
{
    if ( i == NULL || i->data == NULL )
        printf("free_image_double: invalid input image.");
    free( (void *) i->data );
    free( (void *) i );
}
...
image_double new_image_double(unsigned int xsize, unsigned int ysize)
{
    ...
    /* check parameters */
    if ( xsize == 0 || ysize == 0 ) printf("new_image_double: invalid image size.");
...
image_double new_image_double_ptr(unsigned int xsize,
    ...
    if (data == nullptr) printf("new_image_double_ptr: nullptr data pointer.");

    /* get memory */
    image = (image_double) malloc(sizeof(struct image_double_s));
    if (image == nullptr) printf("not enough memory.");
...
static void gaussian_kernel(ntuple_list kernel, double sigma, double mean)
{
    ...
    if ( kernel == NULL || kernel->values == NULL )
        printf("gaussian_kernel: invalid n-tuple 'kernel'.");
    if ( sigma <= 0.0 ) printf("gaussian_kernel: 'sigma' must be positive.");
                                      ...
                                      double sigma_scale )
{
    ...
    if ( in == NULL || in->data == NULL || in->xsize == 0 || in->ysize == 0 )
        printf("gaussian_sampler: invalid image.");
    if ( scale <= 0.0 ) printf("gaussian_sampler: 'scale' must be positive.");
    if ( sigma_scale <= 0.0 )
        printf("gaussian_sampler: 'sigma_scale' must be positive.");

    /* get memory for images */
    if ( in->xsize * scale > (double) UINT_MAX ||
         in->ysize * scale > (double) UINT_MAX )
        printf("gaussian_sampler: the output image size exceeds the handled size.");
                              ...
                              double max_grad )
{
    ...
    /* check parameters */
    if ( in == NULL || in->data == NULL || in->xsize == 0 || in->ysize == 0 )
        printf("ll_angle: invalid image.");
    if ( threshold < 0.0 ) printf("ll_angle: 'threshold' must be positive.");
    if ( list_p == NULL ) printf("ll_angle: NULL pointer 'list_p'.");
    if ( mem_p == NULL ) printf("ll_angle: NULL pointer 'mem_p'.");
    if ( modgrad == NULL ) printf("ll_angle: NULL pointer 'modgrad'.");
    if ( n_bins == 0 ) printf("ll_angle: 'n_bins' must be positive.");
    if ( max_grad <= 0.0 ) printf("ll_angle: 'max_grad' must be positive.");
                      ...
                      double prec )
{
    ...
    if ( angles == NULL || angles->data == NULL )
        printf("isaligned: invalid image 'angles'.");
    if ( x < 0 || y < 0 || x >= (int) angles->xsize || y >= (int) angles->ysize )
        printf("isaligned: (x,y) out of the image.");
    if ( prec < 0.0 ) printf("isaligned: 'prec' must be positive.");
...
{
    /* check parameters */
    if ( in == NULL || out == NULL ) printf("rect_copy: invalid 'in' or 'out'.");
...
static double inter_low(double x, double x1, double y1, double x2, double y2)
{
    /* check parameters */
    if ( x1 > x2 || x < x1 || x > x2 )
        printf("inter_low: unsuitable input, 'x1>x2' or 'x<x1' or 'x>x2'.");
...
static double inter_hi(double x, double x1, double y1, double x2, double y2)
{
    /* check parameters */
    if ( x1 > x2 || x < x1 || x > x2 )
        printf("inter_hi: unsuitable input, 'x1>x2' or 'x<x1' or 'x>x2'.");
...
{
    if ( iter == NULL ) printf("ri_del: NULL iterator.");
    free( (void *) iter );
}
...
{
    /* check input */
    if ( i == NULL ) printf("ri_end: NULL iterator.");
...
{
    /* check input */
    if ( i == NULL ) printf("ri_inc: NULL iterator.");
...
static rect_iter *ri_ini(struct rect *r)
{
    ...
    /* check parameters */
    if ( r == NULL ) printf("ri_ini: invalid rectangle.");

    /* get memory */
    i = (rect_iter *) malloc(sizeof(rect_iter));
    if ( i == NULL ) printf("ri_ini: Not enough memory.");
...
static double rect_nfa(struct rect *rec, image_double angles, double logNT)
{
    ...
    /* check parameters */
    if ( rec == NULL ) printf("rect_nfa: invalid rectangle.");
    if ( angles == NULL ) printf("rect_nfa: invalid 'angles'.");
                         ...
                         image_double modgrad, double reg_angle, double prec )
{
    ...
    /* check parameters */
    if ( reg == NULL ) printf("get_theta: invalid region.");
    if ( reg_size <= 1 ) printf("get_theta: region size <= 1.");
    if ( modgrad == NULL || modgrad->data == NULL )
        printf("get_theta: invalid 'modgrad'.");
    if ( prec < 0.0 ) printf("get_theta: 'prec' must be positive.");
    ...
    if ( double_equal(Ixx, 0.0) && double_equal(Iyy, 0.0) && double_equal(Ixy, 0.0) )
        printf("get_theta: null inertia matrix.");
                         ...
                         double prec, double p, struct rect *rec )
{
    ...
    /* check parameters */
    if ( reg == NULL ) printf("region2rect: invalid region.");
    if ( reg_size <= 1 ) printf("region2rect: region size <= 1.");
    if ( modgrad == NULL || modgrad->data == NULL )
        printf("region2rect: invalid image 'modgrad'.");
    if ( rec == NULL ) printf("region2rect: invalid 'rec'.");
    ...
    if ( sum <= 0.0 ) printf("region2rect: weights sum equal to zero.");
    x /= sum;
    y /= sum;
                         ...
                         double prec )
{
    ...
    /* check parameters */
    if ( x < 0 || y < 0 || x >= (int) angles->xsize || y >= (int) angles->ysize )
        printf("region_grow: (x,y) out of the image.");
    if ( angles == NULL || angles->data == NULL )
        printf("region_grow: invalid image 'angles'.");
    if ( reg == NULL ) printf("region_grow: invalid 'reg'.");
    if ( reg_size == NULL ) printf("region_grow: invalid pointer 'reg_size'.");
    if ( reg_angle == NULL ) printf("region_grow: invalid pointer 'reg_angle'.");
    if ( used == NULL || used->data == NULL )
        printf("region_grow: invalid image 'used'.");
                                 ...
                                 double density_th )
{
    ...
    /* check parameters */
    if ( reg == NULL ) printf("reduce_region_radius: invalid pointer 'reg'.");
    if ( reg_size == NULL )
        printf("reduce_region_radius: invalid pointer 'reg_size'.");
    if ( prec < 0.0 ) printf("reduce_region_radius: 'prec' must be positive.");
    if ( rec == NULL ) printf("reduce_region_radius: invalid pointer 'rec'.");
    if ( used == NULL || used->data == NULL )
        printf("reduce_region_radius: invalid image 'used'.");
    if ( angles == NULL || angles->data == NULL )
        printf("reduce_region_radius: invalid image 'angles'.");
                   ...
                   image_char used, image_double angles, double density_th )
{
    ...
    /* check parameters */
    if ( reg == NULL ) printf("refine: invalid pointer 'reg'.");
    if ( reg_size == NULL ) printf("refine: invalid pointer 'reg_size'.");
    if ( prec < 0.0 ) printf("refine: 'prec' must be positive.");
    if ( rec == NULL ) printf("refine: invalid pointer 'rec'.");
    if ( used == NULL || used->data == NULL )
        printf("refine: invalid image 'used'.");
    if ( angles == NULL || angles->data == NULL )
        printf("refine: invalid image 'angles'.");
                                  ...
                                  image_int *region )
{
    ...
    if ( scale <= 0.0 ) printf("'scale' value must be positive.");
    if ( sigma_scale <= 0.0 ) printf("'sigma_scale' value must be positive.");
    if ( quant < 0.0 ) printf("'quant' value must be positive.");
    if ( ang_th <= 0.0 || ang_th >= 180.0 )
        printf("'ang_th' value must be in the range (0,180).");
    if ( density_th < 0.0 || density_th > 1.0 )
        printf("'density_th' value must be in the range [0,1].");
    if ( n_bins <= 0 ) printf("'n_bins' value must be positive.");
    if ( max_grad <= 0.0 ) printf("'max_grad' value must be positive.");
    ...
    if ( reg == NULL ) printf("not enough memory!");
...
double * lsd(int *n_out, double *img, int X, int Y, double gradientThreshold, double log_eps){
    ...
    ntuple_list ntl = lsd_scale(image, scale, sigma_scale, ang_th, quant, log_eps,
                                density_th, n_bins, gradientThreshold);
    *n_out = (int) (ntl->size);
    return ntl->values;
}